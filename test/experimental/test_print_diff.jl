...
@testset "print diff" begin
    g1 = ZGroup()
    g2 = ZGroup()
    @test string(StorageTrees.show_diff(; g1, g2)) == ""
    StorageTrees.@test_equal g1 g2
    g1["subgroup"] = ZGroup()
    @test string(StorageTrees.show_diff(; g1, g2)) == """
        "subgroup" in g1 but not in g2
        """
    @test string(StorageTrees.show_diff(; g2, g1)) == """
        "subgroup" in g1 but not in g2
        """
    ...
    @test string(StorageTrees.show_diff(startswith("f"); g1, g2)) == """
        attrs(g2)["bar"] is 3
        collect(g1["subgroup/ds1/"]):
        [1, 2, 3]
        collect(g2["subgroup/ds1/"]):
